class SecondContract::IFLib::Data::Command
  ...
  def default_error
    ...
    if !@direct.empty?
      bits << SecondContract::IFLib::Sys::English.instance.item_list(
        @direct.map{ |d| unparse_noun_phrase(d) }
      )
    end
    if !@indirect.empty?
      bits << SecondContract::IFLib::Sys::English.instance.item_list(
        @indirect.group_by{ |i| i[0] }.map{ |p| p.first.to_s + " " + SecondContract::IFLib::Sys::English.instance.item_list(p.last) }
      )
    end
    if !@instrument.empty?
      bits << SecondContract::IFLib::Sys::English.instance.item_list(
        @instrument.group_by{ |i| i[0] }.map{ |p| p.first.to_s + " " + SecondContract::IFLib::Sys::English.instance.item_list(p.last) }
      )
    end
    bits.join(" ") + "."
  end
